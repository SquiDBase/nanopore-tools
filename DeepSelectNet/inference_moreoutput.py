...
def main(saved_model, slow5_dir, cutoff, subsample_size, sampling_coefficient, batch, pico, label, mad, repeated_norm,
	...
	writer.writerow(["read_id", "predicted_label", "actual_label", "confidence", "accuracy"])
			...
			if read_count % BATCH == 0:
				...
				for predicted_y in predicted_ys:
					...
					writer.writerow([
						...
						rounded_predicted_y,
						LABEL,
						round(predicted_y[0] * 100, 4),
						accuracy
					])
					
					#print("predicted: ", rounded_predicted_y, "Actual: ", LABEL, "Confidence: ", round(predicted_y[0] * 100, 4), "Accuracy: ", accuracy, "reads: ", predicted_read_count,"/", num_reads)
		else:
			rejected_reads += 1
	end = timer()
