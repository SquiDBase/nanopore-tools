...
one Dockerfile, differing only in `dorado` version and baked-in models:

| chemistry | newest `fast` model | image |
|---|---|---|
| R10.4.1 400bps 5 kHz | `dna_r10.4.1_e8.2_400bps_fast@v5.2.0` | `dorado` (2.1.2) |
| RNA004 | `rna004_fast@v6.0.0` | `dorado` (2.1.2) |
| R10.4.1 400bps 4 kHz | `dna_r10.4.1_e8.2_400bps_fast@v4.1.0` | `dorado-legacy` (0.9.6) |
| R9.4.1 | `dna_r9.4.1_e8_fast@v3.4` | `dorado-legacy` (0.9.6) |
| R10.4.1 260bps | `dna_r10.4.1_e8.2_260bps_fast@v4.1.0` | `dorado-legacy` (0.9.6) |
| RNA002 | `rna002_70bps_fast@v3` | `dorado-legacy` (0.9.6) |

`dorado-legacy` is not a stale pin: it is the only line that still basecalls
...
mounted to the container's `/workspace`.

```bash
alias dorado="docker run --rm -it -v $PWD:/workspace ghcr.io/squidbase/dorado:latest dorado"
```
...
- **Mounting Volumes:** When running the Docker command, ensure that your
  ...
  your local system.
- **Supported input format:** pod5 only. FAST5 support was removed upstream
  in dorado 1.0.0 and never restored, including in the 0.9.6 line used here.
